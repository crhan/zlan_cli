...
非 `--quiet`/非 `--json` 时,逐个出口打印实际生效的源地址/端口、出口网卡名与
覆盖的广播目标(及单播探测数),便于排查"设备 IP 不在本机任何网段时为何扫不到"。

**陌生网段设备(出厂默认 IP)**:设备出厂/复位默认 IP 多为 `192.168.1.200`;
若该网段不在本机任一网卡上,自动模式既不会对它发定向广播、也不做单播探测,只剩
`255.255.255.255` 受限广播可能触达——而设备应答需单播回本机源地址,跨子网时它得
查网关,出厂默认网关多半不可达,**能否回来取决于设备固件是否对来源做二层回包,
不保证**。可靠做法是先在网卡上加一个该网段的辅助地址(如 `192.168.1.9/24`),让
本机进入同一子网,自动模式即生成该网段的定向广播 + 单播探测,设备同网段二层直接
应答,无需网关。`--target <网段广播> --bind <本机源>` 只是退路,仍受上述回包限制。
